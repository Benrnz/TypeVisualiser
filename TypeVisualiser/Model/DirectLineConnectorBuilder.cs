...
namespace TypeVisualiser.Model
{
    ...
    internal class DirectLineConnectorBuilder :IConnectorBuilder
    {
        ...
        public ConnectionLine CalculateBestConnection(Area fromArea, Area destinationArea, Func<Area, ProximityTestResult> isOverlappingWithOtherControls)
        {
            ...
            var fromIdealPoint = fromArea.CalculateCircumferenceIntersectionPoint(fromAngle);
            double toAngle = TrigHelper.InverseAngle(fromAngle);
            var toIdealPoint = destinationArea.CalculateCircumferenceIntersectionPoint(toAngle);
            var toLineEnd = toIdealPoint;

            // Leave room for arrow head
            var offset1 = (ArrowHead.ArrowWidth - 2) * Math.Sin(CalculateLineAngleToPreviousAxis(toAngle));
            var offset2 = (ArrowHead.ArrowWidth - 2) * Math.Cos(CalculateLineAngleToPreviousAxis(toAngle));
