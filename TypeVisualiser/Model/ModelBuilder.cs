namespace TypeVisualiser.Model
{
    ...
    internal class ModelBuilder : IModelBuilder
    {
        ...
        public IVisualisableTypeWithAssociations BuildSubject(IVisualisableType type, int depth)
        {
            ...
            var netType = BuildType(type.AssemblyFileName, type.AssemblyQualifiedName);
            var subject = BuildSubject(netType, depth);
        ...
        public IVisualisableTypeWithAssociations BuildSubject(Type type, int depth)
        {
            ...
            var args = new Dictionary<string, object> { { "type", type }, { "depth", depth } };
            visualisableTypeSubject = Factory.GetInstance<IVisualisableTypeWithAssociations>(new ExplicitArguments(args));
        ...
        {
            return BuildSubject(BuildType(assemblyFile, fullTypeName), depth);
        }
        ...
        public IVisualisableType BuildVisualisableType(Type type, int depth)
        {
            ...
            if (cachedType != null)
            {
                if (type.AssemblyQualifiedName != cachedType.AssemblyQualifiedName)
                {
                    ...
                    Debug.Assert(type.AssemblyQualifiedName == cachedType.AssemblyQualifiedName, "Cached type does not match the type required - matching by GUID may not be working as intended. " + type.AssemblyQualifiedName + " != " + cachedType.AssemblyQualifiedName);
                }

                return cachedType;
            }

            cachedType = new VisualisableTypeWithAssociations(type, depth);
